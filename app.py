...
if uploaded_file:
    ...
    position_options = ["CF", "Full Back", "Destroyer CM", "Penalty Box CB"]
    ...
    metric_groups = position_metrics[position]
    group_colors = {
        'Off The Ball': 'darkorange',
        'Attacking': 'royalblue',
        'Possession': 'seagreen',
        'Defensive': 'orange'
    }
    ...
    st.markdown("---")
    st.subheader("Z-Score Rankings")
    z_score_ranks = []
    for player in players:
        row = plot_data[plot_data['Player'] == player]
        if not row.empty:
            try:
                percentiles = row[[m + ' (percentile)' for m in metric_groups]].fillna(0).values.flatten()
                z_scores = (percentiles - 50) / 15
                avg_z = np.mean(z_scores)
                team = row['Team'].values[0] if 'Team' in row else ""
                z_score_ranks.append((player, team, avg_z))
            except:
                continue
    z_score_ranks.sort(key=lambda x: x[2], reverse=True)
    for player, team, score in z_score_ranks:
        st.write(f"{player} ({team}): {score:.2f}")
